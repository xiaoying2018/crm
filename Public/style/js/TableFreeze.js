...
$.fn.FrozenTable = function(iRowHead,iRowFoot,iColLeft){
	...
	var offset_left = oDiv.offset().left;
	...
	if(oTable.width()>oDiv.width() && oTable.height()>oDiv.height()){
		if(iRowHead>0 && iColLeft>0){
			...
			oCloneTable.css("z-index","10");
			oCloneTable.css("left",offset_left);
			oCloneTable.css("top",offset_top);
		}
		if(iRowFoot>0 && iColLeft>0){
			...
			oCloneTable.css("left",offset_left);
			oCloneTable.css("top",(offset_top+oDiv.outerHeight(true)-oCloneTable.outerHeight(true)-17));
		}
	}
	if(iRowHead>0 && oTable.height()>oDiv.height()){
		...
		oCloneDiv.css("z-index","8");
		oCloneDiv.css("left",offset_left);
		oCloneDiv.css("top",offset_top);
	}
	if(iRowFoot>0 && oTable.height()>oDiv.height()){
		...
		oCloneDiv.css("left",offset_left);
		oCloneDiv.css("top",offset_top+oDiv.outerHeight(true)-oCloneTable.outerHeight(true)-17);
	}
