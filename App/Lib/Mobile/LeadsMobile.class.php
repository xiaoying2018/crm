...
class LeadsMobile extends Action {
		...
		if($this->isPost()){
			...
			if($m_leads->create()){
				if($m_leads_data->create()!==false){
					...
					alert('error',$m_leads_data->getError(),U('leads/info'));
				}
			}else{
				alert('error',$m_leads->getError(),U('leads/info'));
			}
	...
	public function edit(){
		...
		if (!$leads || $leads['is_deleted'] == 1) {
			...
			$this->display('Public:role');
			die();
		}elseif(!in_array($leads['owner_role_id'], $this->_permissionRes) && $this->_permissionRes ){
			$this->display('Public:role');
			die();
		}
		$field_list = M('Fields')->where('model = "leads"')->order('order_id')->select();
		...
		if($this->isPost()){
			...
			foreach($field_list as $k=>$v){
				...
				if($v['form_type'] == 'editor'){
					$data_list[$i]['val'] = 'm=index&a=content_info&fields_m=leads&field_name='.$field.'&id='.$leads_id;
