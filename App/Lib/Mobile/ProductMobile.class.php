...
class ProductMobile extends Action {
	...
	public function index(){
		...
		if($this->isPost()){
			...
			foreach($list as $k=>$v){
				...
				$m_stock = M('Stock');
				//产品库存
				$stock_count = $m_stock->where('product_id = %d',$v['product_id'])->getField('amounts');
				$list[$k]['stock_count'] = !empty($stock_count) ? $stock_count : '0';
	...
	public function view(){
		if($this->isPost()){
			...
			$stock_count = empty($stock_count) ?  0 : $stock_count;
			$data_list[$k+1]['库存'] = $stock_count;
			//创建人
			$creator_role_id = $product['creator_role_id'];
			$role_name = M('User')->where(array('role_id'=>$creator_role_id))->getField('name');
			if($role_name){
				$data_list[$k+2]['创建人'] = $role_name;
			}else{
				$data_list[$k+2]['创建人'] = '';
			}
			//获取权限
			$data['permission'] = getpermission(MODULE_NAME);
			$data['data'] = $data_list;
			$data['info'] = 'success';
			$data['status'] = 1;
			//获取产品主图
	...
	public function product_list(){
		if($this->isPost()){
			...
			$p = isset($_POST['p']) ? intval($_POST['p']) : 1 ;
			$list = $d_product->Page($p.',10')->select();
