...
class CustomerMobile extends Action {
		...
		if($this->isPost()){
			...
			if($customer_record_count < $customer_limit_counts){
				if($m_customer->where('customer_id = %d', $customer_id)->save($data)){
					...
					$this->ajaxReturn('领取成功！','领取成功！',1);
				}else{
					$this->ajaxReturn('','领取失败！',2);
				}
			}else{
				$this->ajaxReturn('','领取失败，您的领取次数已超过上限！',2);
			}
		}
	}
	...
	public function index(){
		...
		if($this->isPost()){
			...
			$c_outdays = M('config') -> where('name="contract_outdays"')->getField('value');
			$c_outdays = empty($c_outdays) ? 0 : $c_outdays;
			$contract_outdays = empty($c_outdays) ? 0 : time()-86400*$c_outdays;
			$openrecycle = M('config') -> where('name="openrecycle"')->getField('value');
			...
			$list_data = $d_v_customer->where($where)->order($order)->page($p.',10')->select();
			$list = array();
			foreach($list_data as $k=>$v){
				...
				$list[$k]['owner_name'] = $m_user->where(array('role_id'=>$v['owner_role_id'],'status'=>1))->getField('full_name');
				$list[$k]['creator_name'] = $m_user->where(array('role_id'=>$v['creator_role_id'],'status'=>1))->getField('full_name');
	...
	public function view(){
		...
		$customer['business_count'] = empty($customer['business'])?0:sizeof($customer['business']);
	...
	public function dynamic(){
		if($this->isPost()){
			...
			//合同统计
			$contract_count = D('ContractView')->where('contract.customer_id = %d and contract.is_deleted=0', $customer['customer_id'])->count();
			...
			foreach ($log_list as $key=>$value) {
				...
				if($value['sign'] == 1){
					...
					if($sign_info){
						...
						$sign_customer_name = M('Customer')->where('customer_id = %d',$sign_info['customer_id'])->getField('name');
						$sign_info['sign_customer_name'] = !empty($sign_customer_name)?$sign_customer_name:'';
						$log_list[$key]['sign_data'] = empty($sign_info)?array():$sign_info;
					}else{
						$log_list[$key]['sign_data'] = array();
					}
		...
		if($this->isPost()){
			...
			foreach($field_list as $k=>$v){
				...
				if($v['form_type'] == 'editor'){
					...
					$data_list[$i]['val'] = 'm=index&a=content_info&fields_m=customer&field_name='.$field.'&id='.$customer_id;
					$data_list[$i]['type'] = 13;
			...
			//判断是否客户池,客户池只给看权限
			//获取权限
			if($customer['owner_role_id'] && ($customer['update_time'] >= $outdate || $customer['is_locked'] == 1)){
