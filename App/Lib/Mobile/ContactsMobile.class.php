...
class ContactsMobile extends Action{
	...
	public function index(){
		...
		if($this->isPost()){
			...
			//获取筛选条件
			$fields_list = M('Fields')->where(array('model'=>'contacts','form_type'=>array('in','text,box'),'is_main'=>1))->field('name,field,setting,form_type,input_tips')->select();
				...
				if($customer['owner_role_id'] != 0 && ($customer['update_time'] > $outdate || $customer['is_locked'] == 1) && $is_share ==0 && !in_array($customer_id, $customer_idArr)){
					...
					foreach($field_list as $k=>$v){
						...
						if($v['form_type'] == 'editor'){
							...
							$data_list[$k]['val'] = 'm=index&a=content_info&fields_m=contacts&field_name='.$field.'&id='.$contacts_id;
		...
		if($this->isPost()){
			...
			if(!in_array($customer_id, $customer_idArr)){
				...
				if($m_contacts->create($params)){
					if($m_contacts_data->create($params) !== false){
						$m_contacts->update_time = time();
						...
						$this->ajaxReturn($m_contacts_data->getError(),'修改失败',2);
					}
				}else{
	...
	public function contacts_list(){
		if($this->isPost()){
			...
			if($by == 'task'){
				...
				$m_customer = M('Customer');
				$p = isset($_POST['p']) ? intval($_POST['p']) : 1 ;
				$contacts_list = $m_contacts->where($where)->page($p.',10')->field('contacts_id,name')->select();
				$count = $m_contacts->where($where)->count();
				foreach($contacts_list as $k=>$v){
					$customer_id = $rcc->where('contacts_id = %d',$v['contacts_id'])->getField('customer_id');
					$customer_name = $m_customer->where('customer_id = %d',$customer_id)->getField('name');
					$contacts_list[$k]['customer_name'] = !empty($customer_name) ? $customer_name : '';
				}
				$page = ceil($count/10);
				$data['list'] = empty($contacts_list) ? array() : $contacts_list;
				$data['page'] = $page;
			}else{
				$customer_id = $this->_post('id','intval');
				if(!$customer_id){
					$this->ajaxReturn('参数错误！','参数错误！',2);
				}
				...
				$contacts_list = $m_contacts->where($where)->field('contacts_id,name')->select();
				$data['list'] = empty($contacts_list) ? array() : $contacts_list;
			}
			$this->ajaxReturn($data,'success',1);
		}
