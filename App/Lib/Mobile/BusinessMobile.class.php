...
class BusinessMobile extends Action {
	...
	public function dynamic(){
		if($this->isPost()){
			...
			$business_id = isset($_POST['id']) ? intval($_POST['id']) : 0;			$business = D('BusinessTopView')->where('business.business_id = %d', $business_id)->find();
	...
	public function loglist(){
		if($this->isPost()){
			...
			$business = D('BusinessTopView')->where('business.business_id = %d', $business_id)->find();
			$p = isset($_POST['p']) ? intval($_POST['p']) : 1 ;
			//沟通日志
			$log_ids = M('rBusinessLog')->where('business_id = %d', $business_id)->getField('log_id', true);
		...
		if($this->isPost()){
			...
			if($m_business->create($params)){
				...
				if($business_id = $m_business->add()){
					//客户到期时间
					$m_customer = M('Customer');
					$m_customer->where('customer_id = %d',intval($params['customer_id']))->setField('update_time',time());
					...
					if(intval($params['status_id']) == 100){
						//项目成功，把客户锁定
						$m_customer->where('customer_id = %d', intval($params['customer_id']))->setField('is_locked',1);
					}
	...
	public function editnew(){
		...
		if($this->isPost()){
			...
			$field_list = $fields_list_business;
	...
	public function viewnew(){
		...
		if($this->isPost()){
			...
			$business = $d_business ->where('business.business_id = %d',$business_id)->find();
			if(!$business_id){
				$this->ajaxReturn('参数错误!','参数错误!',2);
			}elseif(!$business) {
				$this->ajaxReturn('商机不存在或已被删除!','商机不存在或已被删除!',2);
			...
			foreach($arr as $key=>$val){
				...
				$setting = '';
				if($val == 'customer_id'){
					$fields_list_business[$key]['type'] = '3';
					$fields_list_business[$key]['form_type'] = 'customer';
					$fields_list_business[$key]['setting'] = '';
				}elseif($val == 'contacts_id'){
					$fields_list_business[$key]['type'] = '5';
					$fields_list_business[$key]['form_type'] = 'contacts';
					$fields_list_business[$key]['setting'] = '';
				}elseif($val == 'status_id'){
					//获取商机状态
			...
			foreach($field_list as $k=>$v){
				...
				if($field != 'contacts_id'){
					if($v['form_type'] == 'editor'){
						...
						$data_list[$i]['val'] = 'm=index&a=content_info&fields_m=business&field_name='.$field.'&id='.$business_id;
						$data_list[$i]['type'] = 13;
