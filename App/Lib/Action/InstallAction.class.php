...
class InstallAction extends Action {	
	...
	public function index(){
		...
		if (!file_exists(getcwd() . "/Public/sql/mxcrm.sql")) {
		...
		if ($_POST['submit']) {
			...
			$webhost = $_POST['WEB_HOST'];
			
			if (empty($webhost)) {
				$warnings[] = L('PLEASE_FILL_IN_THE_WEBHOST');
			}
			...
			if (empty($warnings)) {
				...
				if(file_put_contents(CONF_PATH . "db.php", $db_config_str)){
					$db = M();
                    $sql = file_get_contents(getcwd() . "/Public/sql/mxcrm.sql");
                    $sql = str_replace("mxcrm_", C('DB_PREFIX'), $sql); 
                    ...
                    $db->query('insert into ' . C('DB_PREFIX') . 'user (role_id, category_id, status, name, password, salt, reg_ip, reg_time ,prefixion ,number, full_name) values (1, 1, 1, "'.$name.'", "'.$password.'", "'.$salt.'", "'.get_client_ip().'", '.time().', "K", "K0001", "'.$name.'")'); 
					touch(CONF_PATH . "install.lock");
					$web1 = '{"data":{"url":"';
			        $web2 = '\/"},"status":1}';
			        file_put_contents(CONF_PATH . "url.php", $web1 . $webhost . $web2 );
				}			
	...
	public function upgradeProcess() {
			...
			if( $dir_handle = opendir($dir) ){
				while (false !== ( $file_name = readdir($dir_handle)) ) {
					if($file_name=='.' or $file_name =='..'){
						continue;
					} elseif ($file_name != "mxcrm.sql" && strpos($file_name,'.sql')){
		...
		$contacts_ids = $m_contacts->getField('contacts_id',true);
		$m_contacts_data= M('contacts_data');
		foreach($contacts_ids as $k=>$v){
			$data['contacts_id'] = $v;
			$m_contacts_data ->add($data);
		} 
		$where['gender'] = '男';
		$m_contacts ->where('sex = 0') ->save($where); 
		$gender['gender'] = '女';
		$m_contacts ->where('sex = 1') ->save($gender); 
