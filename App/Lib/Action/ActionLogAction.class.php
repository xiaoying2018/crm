	...
	public function index(){
		...
		foreach($list as $k=>$v){
			...
			if($v['module_name'] == 'examine'){
				$type = $m_module_name->where("$id = %d", $v['action_id'])->getField('type');
				switch($type){
						case 1:$name = '普通审批';break;
						case 2:$name = '请假单';break;
						case 3:$name = '报销单';break;
						case 4:$name = '差旅单';break;
						case 5:$name = '借款单';break;
					}
			}elseif($v['module_name'] == 'contract'){
				$name = $m_module_name->where("$id = %d", $v['action_id'])->getField('number');
			}else{
			...
			if($v['module_name'] == 'log'){
				$view_type = 'mylog_view';
			}else{
				$view_type = 'view';
			}
			$list[$k]['content'] = $v['content'].'---<a href="./index.php?m='.$v[module_name].'&a='.$view_type.'&'.$param_name.'&id='.$v[action_id].'">'.$name.'</a>';
