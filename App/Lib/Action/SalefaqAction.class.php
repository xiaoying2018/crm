...
* 销售FAQ 管理
**/
class SalefaqAction extends Action{
	...
	**/
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('getcurrentstatus')//,'index','add','view','edit','delete','category','categoryadd','categoryedit','categorydelete','excelExport','excelImport'
		);
	...
	**/
	public function index(){
		$d_knowledge = D('SalefaqView'); // 实例化
		import('@.ORG.Page');// 导入分页类
		$where = array();
		$params = array();
		
		$order = "salefaq.update_time desc,salefaq.salefaq_id asc";
		if($_GET['desc_order']){
			$order = 'salefaq.'.trim($_GET['desc_order']).' desc,salefaq.salefaq asc';
		}elseif($_GET['asc_order']){
			$order = 'salefaq.'.trim($_GET['asc_order']).' asc,salefaq.salefaq asc';
		}
		$m_category = M('Salefaq_cate');
		$category_id = intval($_GET['category_id']);
		$where['salefaq.role_id'] = array('in',$this->_permissionRes);
		...
		if(!session('?admin')){
			...
			if($category_id){
				if(!in_array($category_id, $category_ids)){
					...
					$where['salefaq.category_id'] = array('in',$idArray);
				}
			}else{
				if($category_ids){
					$where['salefaq.category_id'] = array('in', $category_ids);
				}else{
					$where['salefaq.category_id'] = array('eq',0);
				}
			}
		...
		if(trim($_GET['act']) == 'excel'){
			if(!checkPerByAction('salefaq','excelexport')){
				alert('error',L('DO NOT HAVE PRIVILEGES'),$_SERVER['HTTP_REFERER']);
			}else{
				$dc_id = $_GET['daochu'];
				if($dc_id !=''){
					$where['salefaq_id'] = array('in',$dc_id);
				}
		...
		$this->tree_code = getSalefaqTreeCode(0,1); //类别选项
		...
		if($this->isPost()){
			...
			if($knowledge->create()){
				...
				if($_POST['submit'] == L('SAVE')) {
					alert('success', L('ARTICLE_ADD_SUCCESS'), U('salefaq/index'));
					alert('success', L('ARTICLE_SAVE_SUCCESS'), U('salefaq/index','id='.$konwledge_id));
				} else {
					alert('success', L('ADD_SUCCESS'), U('salefaq/add'));
				}
			}else{
				$this->error($knowledge->getError(), U('salefaq/index'));
			}
	...
	**/
	public function view(){
		if($_GET['id']){
			$knowledge = M('salefaq');
			$knowledge->where('salefaq_id=%d',$_GET['id'])->setInc('hits');
			$knowledge = $knowledge->where('salefaq_id = %d ', $_GET['id'])->find();
			// 类别权限
			if(!session('?admin')){
				$category_ids = M('SalefaqCate')->where('to_department like "%,'.session('department_id').',%"')->getField('category_id', true);
				if(!in_array($knowledge['category_id'],$category_ids)){
					alert('error',L('DO NOT HAVE PRIVILEGES'),U('salefaq/index'));
				}
			}
			// 岗位权限
	...
	**/
	public function edit(){
		$m_knowledge = M('Salefaq');
		$knowledge_id = $this->_get('id','intval', intval($_POST['salefaq_id']));
		if(!$knowledge = $m_knowledge->where('salefaq_id = %d',$knowledge_id)->find()){
			$this->error(L('PARAMETER_ERROR'));
		...
		if($this->isPost()){
			...
			if($m_knowledge->create()){
				...
				if($m_knowledge->save()){
					if($_POST['submit'] == L('SAVE')) {
//                        alert('success', "编辑成功！", U('salefaq/view','id='.I('post.salefaq_id'))); TODO 如果编辑后希望直接预览
                        alert('success', "编辑成功！", U('salefaq/index'));
					} else {
						alert('success', L('SAVE_SUCCESS_CONTINUE_INPUT'), U('salefaq/add'));
					}
				} else {
	...
	**/
	public function delete(){
		$knowledge = M('Salefaq');
		$knowledge_idarray = $_POST['salefaq_id'];
		if (is_array($knowledge_idarray)) {
			if($this->_permissionRes){
				foreach ($knowledge_idarray as $v) {
					if (!$knowledge->where('salefaq_id = %d and role_id in (%s)', $v, implode(',', $this->_permissionRes))->find()){
						$this->ajaxReturn('',L('DONOT_HAVE_PERMISSIONS_ONLY_AUTHOR_CAN_DELETE'),0);
					}
				}
			}
			if ($knowledge->where('salefaq_id in (%s)', join(',', $knowledge_idarray))->delete()) {
				$this->ajaxReturn('',L('DELETED SUCCESSFULLY'),1);
			} else {
				$this->ajaxReturn('',L('DELETE FAILED CONTACT THE ADMINISTRATOR'),0);
			}
		} elseif($_GET['id']) {
			if ($this->_permissionRes && !$knowledge->where('salefaq_id = %d and role_id in (%s)', $_GET['id'], implode(',', $this->_permissionRes))->find()){
				$this->ajaxReturn('',L('DONOT_HAVE_PERMISSIONS_ONLY_AUTHOR_CAN_DELETE'),0);
			}
			// 知识类别权限
			if(!session('?admin')){
			...
			if($knowledge->where('salefaq_id = %d', $_GET['id'])->delete()){
				$this->ajaxReturn('',L('DELETED SUCCESSFULLY'),1);
			}else{
				$this->ajaxReturn('',L('DELETE FAILED CONTACT THE ADMINISTRATOR'),0);
			}
	...
	**/
	public function categoryEdit(){
		if($_GET['id']){
			...
			$knowledge_category = M('SalefaqCate');
			$this->assign('tree_code', getSalefaqKnowDepartmentTreeCode($category_id, 0, 1));
			$category_list = $knowledge_category -> select();
			$this->assign('category_list', getSubCategory(0, $category_list, ''));
			$this->knowledge_category =$knowledge_category->where('category_id = %s',$_GET['id'])->find();
            // 岗位权限
//            if(!in_array($this->knowledge_category['role_id'], $this->_permissionRes)) die(L('DO NOT HAVE PRIVILEGES'));
			$this->display();
		}elseif($this->isPost()){
			if(!trim($_POST['name'])){
				alert('error','请填写分类名！',$_SERVER['HTTP_REFERER']);
			...
			if($knowledge_category ->where('category_id =%d',$_POST['category_id'])->save()){
				alert('success',L('UPDATE_CATEGORY_INFO_SUCCESS'),U('salefaq/category'));
			}else{
	...
	**/
	public function excelExport($knowledgeList=false){
		...
		$objProps->setTitle("mxcrm Salefaq");
		$objProps->setSubject("mxcrm Salefaq Data");
		$objProps->setDescription("mxcrm Salefaq Data");
		$objProps->setKeywords("mxcrm Salefaq");
        ...
        header("Content-Disposition:attachment;filename=mxcrm_salefaq_".date('Y-m-d',mktime())."_".$current_page.".xls");
		...
		if($this->isPost()){
			if (isset($_FILES['excel']['size']) && $_FILES['excel']['size'] != null) {
				...
				if (!is_dir($dirname) && !mkdir($dirname, 0777, true)) {
					alert('error', L('ATTACHMENTS TO UPLOAD DIRECTORY CANNOT WRITE'), U('salefaq/index'));
				}
				$upload->savePath = $dirname;
				if(!$upload->upload()) {
					alert('error', $upload->getErrorMsg(), U('salefaq/index'));
				}else{
			...
			if(is_array($info[0]) && !empty($info[0])){
				...
				alert('error', L('UPLOAD FAILED'), U('salefaq/index'));
			}
			import("ORG.PHPExcel.PHPExcel");
			...
			if ($allRow <= 1) {
				alert('error', L('UPLOAD A FILE WITHOUT A VALID DATA'), U('salefaq/index'));
			} else {
				for($currentRow = 3;$currentRow <= $allRow;$currentRow++){
					...
					$category_id = M('SalefaqCate')->where('name = "%s"' ,trim($category))->getField('category_id');
					if($category){
						if($category_id > 0){
							$data['category_id'] = $category_id;
						} else {
							if($this->_post('error_handing','intval',0) == 0){
								alert('error', L('IMPORT_FAILY_SOURCE_NOT_EXIST',array($currentRow,$category)), U('salefaq/index'));
							}else{
					...
					if (!$m_knowledge->add($data)) {
						if($this->_post('error_handing','intval',0) == 0){
							alert('error', L('IMPORT_FAILY_SOURCE',array($currentRow)), U('salefaq/index'));
						}else{
				...
				alert('success', $error_message .L('IMPORT SUCCESS'), U('salefaq/index'));
			}
		}else{
			$this->category_list = getSubCategory(0, M('SalefaqCate')->select(), '');
			$this->display();
		}
	}
}
